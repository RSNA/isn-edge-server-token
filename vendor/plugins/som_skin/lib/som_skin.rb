...
module SomSkin
  ...
  class TabBuilder
    ...
    def tab(name, *args, &block)
      puts "Tab var: #{@active_tab_var}\nValue: #{params[@active_tab_var]}"
      name == params[@active_tab_var] ?  li_class = "active" : li_class = ""
      if block_given?
    ...
    def tab_link_to(*args)
      ...
      args[1].merge!({@active_tab_var => args[0]})
      tab(args[0]) { helpers.link_to(args[0],args[1]) }
    end
  ...
  module Helpers
    ...
    def som_tab_builder(active_tab_var, options={}, *args, &block)
      ...
      concat(smartie_tab_builder.render)
    end
    ...
    def render_som_block(args={}, options={}, &block)
      ...
      if eval(options[:if])
        concat(render(:partial => "layouts/som_skin/partials/#{args[:partial]}", :locals => {:heading => args[:heading], :body => capture(&block)}))
      end
    end
    ...
    def format_radiology_report(report_text)
      text = report_text.gsub(/(.*[\.^])(.*?findings were discussed with.*?$)(.*)/im) {|match| $1 + content_tag(:span, $2, :class => "findings-discussed") + $3}
      text = nl2br(text)

      ["indication","comparison","technique","findings"].each do |kword|
      ...
      if text.match(/(impression\:)(.*)(end of impression[\:\.]*)/im)
        ...
        text.gsub!(/(impression\:)(.*)/im) {|match| content_tag(:p, content_tag(:span, $1, :class => "report-heading") + $2, :class => "report-impression") }
      end
