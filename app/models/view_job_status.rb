class ViewJobStatus < ActiveRecord::Base
  ...
  def self.filter(string)
    query = self.joins("LEFT JOIN (jobs JOIN exams ON (jobs.exam_id = exams.exam_id)) ON (jobs.job_id = v_job_status.job_id) LEFT JOIN (job_sets JOIN patients ON (patients.patient_id = job_sets.patient_id)) ON job_sets.job_set_id = jobs.job_set_id").order("last_transaction_timestamp DESC").limit(100)
